...
FROM_WPP           = os.environ.get("FROM_WPP", "whatsapp:+14155238886")
TO_WPP             = os.environ.get("TO_WPP", "whatsapp:+55XXXXXXXXXXX")
...
def pensar_autonomamente():
    ...
    try:
        resp = gerar_resposta_com_memoria("saito", pergunta)
        enviar_whatsapp(TO_WPP, resp)
    except Exception as e:
        write_memory({
            "timestamp": datetime.now(timezone.utc).isoformat(),
...
@app.route('/')
def index():
    return "✅ Kaizen está rodando (Autonomia + Memória unificada + WhatsApp + Monitor)"
