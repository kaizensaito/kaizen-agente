...
def criar_evento_calendar(title: str, start_dt: datetime, end_dt: datetime):
    ...
    logging.info(f"[Calendar] criado evento {ev['id']} – {title}")
    return ev
...
def criar_tarefa_trello(title: str, desc: str = "", due_days: int = 1):
    ...
    try:
        ...
        r.raise_for_status()
        logging.info(f"[Trello] criado card → {title}")
    except Exception:
...
def gerar_relatorio_performance(chat_id: str) -> str:
    ...
    for m in last:
        ...
        r = m["resposta"]
        lines.append(f"🕐 {t}\n📥 {e}\n📤 {r}")
    body = "\n\n".join(lines) if lines else "Nenhuma interação ainda."
    return f"📊 Últimas interações:\n\n{body}"
    ...
    try:
        ...
        if "relatório de performance" in low:
            ...
            try:
                ...
                resp = "Desculpe, indisponível agora. Tente mais tarde."
...
def ask():
    ...
    try:
        ...
        reply = "Desculpe, indisponível agora. Tente mais tarde."
    return jsonify(reply=reply)

# ─── CICLOS & MONITORAMENTO ───────────────────────────────────────────────────
def pensar_autonomamente():
    h = datetime.now(CLIENT_TZ).hour
    if   5 <= h < 9:    p = "Bom dia. Que atitude proativa tomaria hoje?"
    elif 12 <= h < 14:  p = "Hora do almoço. Gere insight produtivo."
    elif 18 <= h < 20:  p = "Fim de expediente. O que aprendeu?"
    else:               p = "Execute algo útil baseado no histórico."
    try:
